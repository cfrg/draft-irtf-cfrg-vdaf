...
import itertools
from sagelib.common import ERR_DECODE, ERR_INPUT, I2OSP, OS2IP, TEST_VECTOR, \
                           VERSION, Bytes, Error, Unsigned, Vec, byte, \
                           format_custom, gen_rand, vec_add, vec_neg, \
                           vec_sub, xor
...
class IdpfPoplar(Idpf):
    ...
    def gen(IdpfPoplar, alpha, beta_inner, beta_leaf):
        ...
        # REMOVE ME
        if TEST_VECTOR:
            init_seed[1] = bytes([0x02] * IdpfPoplar.Prg.SEED_SIZE)
