...
from sagelib.idpf import Idpf, test_idpf_exhaustive
...
class IdpfPoplar(Idpf):
    ...
    def eval(IdpfPoplar, agg_id, public_share, init_seed, level, prefixes):
        ...
        for prefix in prefixes:
            ...
            # The Aggregator's output share is the value of a node of the IDPF
            # tree at the given `level`. The node's value is computed by
            # traversing the path defined by the candidate `prefix`. Each node
            # in the tree is represented by a seed (`seed`) and a set of control
            # bits (`ctrl`).
            seed = init_seed
            ctrl = Field2(agg_id)
            for current_level in range(level+1):
                ...
                # Implementation note: Typically the current round of candidate
                # prefixes would have been derived from aggregate results
                # computed during previous rounds. For example, when using
                # `IdpfPoplar` to compute heavy hitters, each `prefix` would be
                # the prefix of a string whose hit count exceeded the given
                # threshold. (See [BBCGGI21, Section 5.1].) In this case, part
                # of the path would have already been traversed.
                #
                # Re-computing nodes along previously traversed paths is
                # wasteful. Implementations can eliminate this added complexity
                # by caching nodes (i.e., `(seed, ctrl)` pairs) output by
                # previous calls to `eval_next()`.
                (seed, ctrl, y) = IdpfPoplar.eval_next(
                    seed, ctrl, correction_words[current_level], current_level, bit)
            out_share.append(y if agg_id == 0 else vec_neg(y))
        return out_share

    # Compute the next node in the IDPF tree along the path determined by a
    # candidiate prefix. The next node is determined by `bit`, the bit of the
    # prefix corresponding to the next level of the tree.
    #
    # TODO Consider implementing some version of the optimization discussed at
    # the end of [BBCGGI21, Appendix C.3]. This could on average reduce the number of
    # AES calls by a constant factor.
    @classmethod
    def eval_next(IdpfPoplar, prev_seed, prev_ctrl, correction_word, level, bit):
