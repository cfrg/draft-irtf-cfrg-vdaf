...
class Sum(
    ...
    def __init__(self, field: type[F], max_measurement: int):
        ...
        The range checked measurement is the bit-encoding of `offset`
        plus the measurement. Observe that only measurements between `0`
        and `max_measurement` incluisve can be encoded this way with
        `bits` bits.
