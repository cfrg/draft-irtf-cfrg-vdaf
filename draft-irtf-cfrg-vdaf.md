...
is fully linear, which means the coordinates of the polynomial can be secret
...
1. The Client, given its measurement `x`, constructs the lowest degree
   ...
   shares of the coordinates of `p` to each of the Aggregators.
...
the field are used. This allows using the
number theoretic transform {{Pol71}} for efficiency.
...
recorded values.
Note that these polynomials are in the value (Lagrange) representation
by construction, and the gadget polynomials parsed from the proof share
are similarly transmitted in the value (Lagrange) representation.
...
Polynomial evaluation in the value (Lagrange) representation is
