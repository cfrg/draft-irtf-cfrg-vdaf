...
* `Field.ntt(p: list[F], n: int, set_s: bool = False) -> list[F]` returns
  ...
  representation of the polynomial, see {{poly_repr}});
...
### Polynomial Representation {#poly_repr}
...
- `poly_mul(self, p: list[F], q: list[F]) -> list[F]` multiplies two
  ...
  The length of inputs must be equal and be a power of two.

- `poly_eval(self, p: list[F], x: F) -> F` evaluates a polynomial
  `p` (in the Lagrange basis) at `x` in linear time.
  Avoids conversion to the monomial basis as shown below.
...
the monomial and the Lagrange basis ({{poly_repr}}).
