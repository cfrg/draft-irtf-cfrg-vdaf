...
* `Daf.measurement_to_input_shares(input: Measurement) -> Vec[Bytes]` is the
  ...
  The length of the output MUST be `SHARES`.
...
syntactic changes, as well as a new Setup phase for distributing long-lived
verification parameters.

Overall execution of a VDAF comprises the following stages:

* Setup - Generating shared parameters for Sharding and Preparation
...
Sharding is identical to DAFs (cf. {{sec-daf-shard}}) except that the
...
The data flow for this stage is the same as illustrated in {{shard-flow}}.
