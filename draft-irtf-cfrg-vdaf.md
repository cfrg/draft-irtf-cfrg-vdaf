...
* `Vdaf.shard(measurement: Measurement, nonce: bytes, rand: bytes) ->
  ...
  * `nonce` MUST have length equal to `NONCE_SIZE` and MUST be generated using
    CSPRNG. (See {{security}} for details.)
...
* `Vdaf.decode_input_share(agg_id: int, encoded: bytes) ->
...
* `Field.zeros(length: int) -> output: list[Field]` returns a vector of
  zeros.
  ...
  Post-conditions:
    ...
    * The length of `output` MUST be `length`.

* `Field.rand_vec(length: int) -> output: list[Field]` returns a vector of
...
~~~
def vec_sub(left: list[Field], right: list[Field]):
    """
    Subtract the right operand from the left and return the result.
    """
    return list(map(lambda x: x[0] - x[1], zip(left, right)))

def vec_add(left: list[Field], right: list[Field]):
...
* `Flp.prove(meas: list[Field], prove_rand: list[Field], joint_rand: list[Field])
  -> list[Field]` is the deterministic proof-generation algorithm run by the
...
* `Flp.query(meas: list[Field], proof: list[Field], query_rand: list[Field],
  joint_rand: list[Field], num_shares: int) -> list[Field]` is the
...
* `Flp.decide(verifier: list[Field]) -> bool` is the deterministic decision
...
* `Flp.encode(measurement: Measurement) -> list[Field]` encodes a raw measurement
...
* `Flp.truncate(meas: list[Field]) -> list[Field]` maps an encoded measurement
...
* `Flp.decode(output: list[Field], num_measurements: int) -> AggResult`
  maps a sum of aggregate shares to an aggregate result.

  Pre-conditions:

  * The length of `output` MUST be `OUTPUT_LEN`.
  * `num_measurements` MUST equal he number of measurements that contributed to
    the `output`.
...
* `Valid.encode(measurement: Measurement) -> list[Field]` returns a vector of
  length `MEAS_LEN` representing a measurement.

* `Valid.truncate(meas: list[Field]) -> list[Field]` returns a vector of length
  `OUTPUT_LEN` representing an aggregatable output.

* `Valid.decode(output: list[Field], num_measurements: int) -> AggResult`
...
* `Idpf.gen(alpha: int, beta_inner: list[list[FieldInner]], beta_leaf:
  ...
  the values `beta`, and a binder string.
  ...
  Pre-conditions:
    ...
    * `beta_inner` MUST have length `Idpf.BITS - 1`.
    * `beta_inner[level]` MUST have length `Idpf.VALUE_LEN` for each `level` in
       `range(Idpf.BITS)`.
    * `beta_leaf` MUST have length `Idpf.VALUE_LEN`.
    * `rand` MUST be generated by a CSPRNG and have length `Idpf.RAND_SIZE`.
    * `binder` MUST be chosen MUST be chosen uniformly at random by the Client
      (see {{nonce-requirements}}).
...
* `Idpf.eval(agg_id: int, public_share: bytes, key: bytes, level:
  int, prefixes: tuple[int, ...], binder: bytes) -> Output` is the
  ...
  * `agg_id` MUST be in `range(Idpf.SHARES)` and match the index of `key` in
    the sequence of IDPF keys output by the Client.
    share output by the Client.
  * `level` MUST be in `range(0, Idpf.BITS)`.
...
| `AggResult`       | `list[int]`                          |
| `PrepState`       | `tuple[bytes, int, Idpf.FieldVec]`   |
