...
def prep_init(Poplar1, verify_key, agg_id, agg_param,
    ...
    if level < Poplar1.Idpf.BITS - 1:
        corr_xof = Poplar1.Xof(
            corr_seed,
            Poplar1.domain_separation_tag(USAGE_CORR_INNER),
            byte(agg_id) + nonce + to_be_bytes(self.threshold),
        )
...
def is_valid(Poplar1, agg_param, previous_agg_params):
    ...
    (level, prefixes, prefix_counts) = agg_param
    (last_level, last_prefixes, last_prefix_counts) = previous_agg_params[-1]
    ...
    # Save last level's prefixes and counts in a dict.
    last_prefix_count_dict = {}
    for (i, prefix) in enumerate(last_prefixes):
      last_prefix_count_dict[prefix] = last_prefix_counts[i]

    # Check that prefixes are suffixes of the last level's prefixes,
    # and that the counts of siblings sum up to at most the ancestor's count.
    for (i, prefix) in enumerate(prefixes):
      last_prefix = get_prefix(prefix, level, last_level)
      if last_prefix not in last_prefix_count_dict:
        return False  # Current prefix not a suffix of last level's prefixes.
      if last_prefix_count_dict[last_prefix] < prefix_counts[i]:
        return False  # Count exceeds last level's count.
      last_prefix_count_dict[last_prefix] -= prefix_counts[i]
