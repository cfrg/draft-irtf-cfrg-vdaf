...
of measurements generated by Clients. An aggregation function includes an
optional "aggregation parameter", which is selected by the Collector and
disseminated among the Aggregators. The semantics of this parameter is specific
to the aggregation function, but in general it is used to represent the set of
"queries" that can be made on the measurement set.
...
* `Daf.prep(agg_param: AggParam, input_share: Bytes) -> OutShare` is the
  preparation algorithm. It takes as input an input share generated by a Client
  and the aggregation parameter selected by the Collector and returns an output
  share.
...
garbled aggregate result. Note that, before the Preparation stage can begin, the
Aggregators are required to exchange a long-lived shared secret. See
{{sec-vdaf-prepare}} for details.
...
| `ROUNDS`          | Number of rounds of communication during the Preparation stage, at least one ({{sec-vdaf-prepare}}) |
...
Sharding is identical to DAFs (cf. {{sec-daf-shard}}):
...
* `Vdaf.prep_init(verify_key: Bytes, agg_id: Unsigned, agg_param: AggParam,
  nonce: Bytes, input_share: Bytes) -> Tuple[Prep, Bytes]` is the deterministic
  ...
  preparation state and its output prepare-message share.

  The length of `verify_key` MUST be `VERIFY_KEY_SIZE`. It is up to the
  application to arrange for the distribution of the verification key among the
  Aggregators prior to the start of this phase of VDAF evaluation.
  ...
  integer in range `[0, SHARES)` that matches the index of `input_share` in the
  sequence of input shares output by the Client.
...
* `Vdaf.prep_next(prep: Prep, inbound: Bytes) -> Union[Tuple[Prep,
  ...
  algorithm is the inbound preparation message.
