...
* Poplar1: Move prefix uniqueness and ordering checks from `prep_init()` to
  `is_valid()`.
...
* `assert_power_of_2(x: int) -> int` returns `math.ceil(math.log2(n))` if `x`
  is a positive power of two and raises an exception otherwise.
...
In the case of VDAFs ({{vdaf}}), this process is instead called "verification"
...
As in DAFs, sharding is bound to the application context via the application
...
This is referred to as "verification state", usually written as `verify_state`
for short.

During each round, each Aggregator broadcasts a message called a "verifier
share", usually written `verifier_share`. The verifier shares are then combined
into a single message called the "verifier message", or `verifier_message`. The
verifier message is computed from public information and therefore MAY be
computed by any one of the Aggregators.
...
Just as DAF preparation involves an aggregation parameter
...
* `vdaf.verify_init(verify_key: bytes, ctx: bytes, agg_id: int, agg_param:
  ...
  Pre-conditions:
    ...
    * `agg_id` MUST be the integer in the range `[0, vdaf.SHARES)` that matches
      the index of `input_share` in the sequence of input shares output by the
      Client.
    * `nonce` MUST have length `vdaf.NONCE_SIZE`.

* `vdaf.verifier_shares_to_message(ctx: bytes, agg_param: AggParam,
  verifier_shares: list[VerifierShare]) -> VerifierMessage` is the
  ...
  verifier message consumed by each Aggregator to start the next round.

* `vdaf.verify_next(ctx: bytes, verify_state: VerifyState, verifier_message:
  VerifierMessage) -> tuple[VerifyState, VerifierShare] | OutShare` is the
  deterministic verification state update algorithm run by each Aggregator. It
  updates the Aggregator's verification state (`verify_state`) and returns
  either its next verification state and verifier share for the next round or,
  if this is the last round, its output share.
...
public `verifier_shares_to_message()` algorithm and broadcast to the
...
{{vdaf-verification-state-machine}}, except for the Leader's initial
...
state is `Continued` or `FinishedWithOutbound`, then the state includes an
...
next verifier message (via `vdaf.verifier_shares_to_message()`) and broadcasts
it to the Helpers. At this point, each Aggregator runs `vdaf.verify_next()`
...
* `Field.gen() -> Self` is a class method that returns the generator of a large
  ...
  subgroup MUST be a power of two.
...
* `Field.nth_root(n: int) -> F` returns the principal `n`-th root of
  unity that is uniquely determined as `Wn = Field.gen() ** (GEN_ORDER // n)`
  to ensure interoperability.
  Note that `n` must be a power of two such that `1 <= n <= GEN_ORDER`.

* `Field.nth_root_powers(n: int) -> list[F]` returns a list `v` with
  the first `n` powers of the principal `n`-th root of unity.
  It sets `v[i] = Wn ** i`, such that `Wn = Field.nth_root(n)`.

* `Field.ntt(p: list[F], n: int, set_s: bool = False) -> list[F]` returns
  a list `v` with the evaluations of a polynomial `p`
  (given its coefficients) at `n` points.
  When `set_s=False`, it sets `v[i] = p(Wn ** i)` (this is the Lagrange
  representation of the polynomial, see {{poly-repr}});
  otherwise, it sets `v[i] = p(s * (Wn ** i))`,
  such that `Wn = Field.nth_root(n)`, and `s = Field.nth_root(2 * n)`.
...
* `Field.inv_ntt(v: list[F], n: int) -> list[F]` returns the coefficients of
  ...
  powers of the principal `n`-th root of unity. (This is the monomial
  representation of the polynomial, see {{poly-repr}}.)
...
### Polynomial Representation {#poly-repr}

Two standard representations of polynomials are described.
While it is possible to convert between representations
(using the NTT algorithm), polynomials are maintained in the most
efficient representation according to the target operation.

#### Monomial Basis

The monomial basis is the standard way of representing a polynomial,
where each element of the list corresponds to one of their coefficients
stored in ascending degree order,
that is, starting with the coefficient of the constant term.

The following function denotes polynomial evaluation in the
coefficient representation.

- `poly_eval(field: type[F], p: list[F], x: F) -> F` returns the
  evaluation of the polynomial `p` (in the monomial basis) at `x`.
  The `field` parameter is the class object for `F` and is used by
  the implementation to construct field elements. (See {{field}}.)

#### Lagrange Basis

In the Lagrange basis, polynomials are represented as a list of
evaluations over a set of points.
This set is fixed to the first `n` powers of the
principal `n`-th root of unity.
So any polynomial `p` of degree less than `n` is represented as a
list `v` of `n` values such that `v[i] = p(Wn ** i)`,
where `Wn = Field.nth_root(n)`.

The `Lagrange` class defines operations over polynomials in the
Lagrange basis. It implements the following methods:

- `poly_mul(self, p: list[F], q: list[F]) -> list[F]` multiplies two
  polynomials, where the input and output polynomials are in the Lagrange basis.
  The inputs must have lengths that are equal and a power of two.

- `poly_eval(self, p: list[F], x: F) -> F` evaluates a polynomial
  `p` (in the Lagrange basis) at `x` in linear time.
  It avoids conversion to the monomial basis as shown below.

- `poly_eval_batched(self, polys: list[list[F]], x: F) -> list[F]` evaluates
  every polynomial in the input list at `x`.
  This function runs faster than invoking `poly_eval` multiple times
  because some intermediate calculations are shared.

- `extend_values_to_power_of_2(self, p: list[F], n: int)` appends evaluations
  to the polynomial `p` (in-place) until the number of evaluations is `n`.
  The output length `n` must be a power of two.

- `double_evaluations(self, p: list[F]) -> list[F]` returns a list of
  `2*n` evaluations of a polynomial `p` given `n=len(p)` evaluations.
  The length of the input must be a power of two.

An instance of the `Lagrange` class is initialized with an NTT-friendly field.
For algorithm derivations, refer to {{Faz25}}.
...
`verifier_shares_to_message()` algorithm ({{sec-vdaf-verify}}).
...
invalid, then as long as `joint_rand` and `query_rand` are
generated randomly using a uniform distribution,
the output is `False` with high probability. False positives are
...
1. The Client, given its measurement `x`, constructs the lowest degree
   ...
   shares of `p` to each of the Aggregators.
...
Third, rather than defining polynomials at inputs `1`, `2`, ..., `j`,
where `j` is the `j`-th invocation of the gadget, the roots of unity for
...
the monomial and the Lagrange basis ({{poly-repr}}).
...
faster by using the algorithm by {{Faz25}} without resorting to
...
prefix; `num_prefixes`, the number of prefixes to evaluate; and
...
: A list of objects with the following schema:
    ...
    `verifier_messages`:
    : The expected list of verifier messages for each round of
...
: The type of operation to be performed. This is one of "shard", "verify_init",
  "verifier_shares_to_message", "verify_next", "aggregate", or "unshard".
