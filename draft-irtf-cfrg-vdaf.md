...
class Continued(State, Generic[PrepState]):
    def __init__(self, prep_state: PrepState):
...
Procedure `decode` decodes the inbound message and returns the variant
(`initialize`, `continue`, or `finalize`) and the sequence of fields. Procedure
`ping_pong_transition` takes in the prep shares, combines them into the prep
message, and computes the next prep state of the caller:
