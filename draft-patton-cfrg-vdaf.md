...
* `vdaf_setup() -> (public_param, verify_params: Vec[Bytes])` is the randomized
  ...
  revealed to the Clients, Collector or other Aggregators. The parameters MAY
  be reused across multiple VDAF evaluations. See [Security](#security) for a
  discussion of the security implications this has depending on the threat
  model.
