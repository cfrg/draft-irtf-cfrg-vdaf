...
* `SHARES: Unsigned` is the number of Aggregators for which the VDAF is
  defined.

* `ROUNDS: Unsigned` is the number of rounds of communication during the
  preparation phase. (The communication pattern is derfined in
  {{sec-vdaf-prepare}}.)

In addition, the VDAF provides the following types:

* `Measurement` is the type measurement type used by a client during the
  sharding phase ({{sec-vdaf-shard}}).

* `PublicParam` is the type of public parameter used by a Client during the
  sharding phase.

* `VerifyParam` is the type of verification parameter used by each
  Aggregator during the preparation phase ({{sec-vdaf-prepare}}).

* `AggParam` is the type of aggregation parameter used by each Aggregator
  during the preparation phase.

* `OutShare` is the type of each output share recovered by the Aggregators
  during the preparation phase.

* `AggShare` is the type of result of combining a sequence of output shares
  during the aggregation phase ({{sec-vdaf-aggregate}}).

* `AggResult` is the type of aggregate result ({{sec-vdaf-unshard}}).

* `Prep` represents the state of an Aggregator during the preparation
...
of an abstraction base class `Flp` that specifies the set of methods, constants,
and types a concrete FLP must provide.

First, a concrete FLP defines the following constants:

* `PROVE_RAND_LEN: Unsigned` is the length of the prover randomness, i.e., the
  amount of randomness consumed by the prover. This number of random field
  elements are provided to the prover for generating the proof. (See below for
  an explanation.)

* `QUERY_RAND_LEN: Unsigned` is the length of the query randomness, i.e., the
  amount of randomness consumed by the verifier.

* `JOINT_RAND_LEN: Unsigned` is the length of the joint randomness in number of
  field elements. The joint randomness is the randomness consumed by by both the
  prover and verifier.

* `INPUT_LEN: Unsigned` is the number of field elements comprising an input. A
  measurement is encoded as a vector of this length before the proof is
  generated. (See {{flp-encode}} for details.)

* `OUTPUT_LEN: Unsigned` is the length of the aggregable output. (See
  {{flp-encode}}.)

* `PROOF_LEN: Unsigned` is the length of the proof in number of field elements.

* `VERIFIER_LEN: Unsigned` is the length of the verifier message generated by
  querying an input and proof. (See below for an explanation.)

A concerte FLP also defines an associated type:

* `Field` is the finite field for which the FLP is defined. This object is
  compatible with the interface described in {{field}}.
...
| `AggParam`    | `None`                   |
| `Prep`        | `Tuple[Vec[Flp.Field], Bytes, Bytes]` |
