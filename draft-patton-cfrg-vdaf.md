...
* Validation - Validating the correctness of an input share and producing the
...
Sharding and validation are done once per measurement input.  Aggregation and
...
To validate the correctness of the input shares, the Aggregators interact with
...
~~~~
    ...
    input_share_0  input_share_1       input_share_[SHARES-1]
      ...
      V              V                   V
    +-----------+  +-----------+       +-----------+
    | eval init |  | eval init |       | eval init |
    +-----------+  +------------+      +-----------+
      |              |              ...  |
      V              V                   V
    +-----------+  +-----------+       +-----------+
    | eval next |  | eval next |       | eval next |
    +-----------+  +-----------+       +-----------+
      |              |              ...  |
      ====================================
      |              |                   |
...
~~~~
{: #eval-flow title="VDAF validation process on the input shares for a single
...
* `validation_init(verify_param, agg_param, nonce, input_share) ->
  ValidationState` is the deterministic validation-state initialization
...
* `validation_next(state, inbound: Vec[Bytes]) -> (next_state, outbound)` is the
  deterministic validation-state update algorithm run by each Aggregator. It
...
The validation-state update accomplishes two tasks that are essential to most
...
each Aggregator runs the validation-state update algorithm once and immediately
