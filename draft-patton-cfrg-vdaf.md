...
with one another over `ROUND` rounds.  In the first round, each aggregator
...
* `PrepState(verify_param, agg_param, nonce, input_share)` is the deterministic
  ...
  (`input_share`). Its outputs is the Aggregator's initial preparation state.

* `PrepState.next(inbound: Vec[Bytes]) -> outbound` is the deterministic
  ...
  from the previous round or, if this is the first round, an empty vector.
...
[AGJOP21] it is necessary for the Aggregators to interact in order to recover
