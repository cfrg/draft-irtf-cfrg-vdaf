...
The way `prio3` ensures privacy is quite simple: the Client shards its input `x`
vector into a number of additive secret shares, one for each Aggregator.
Aggregators sum up their vector shares locally, and once enough shares have been
aggregator, it sends the aggregate vector to the Collector, who recovers the
aggregate by adding up all of the vector shares.

The main challenge is ensuring that the shares shares generated by each Client
add up to a valid input, i.e., that `C(x)=0`. To accomplish this task,
...
### Fully Linear Probabilistically Checkable Proofs (PCPs) {#pcp}
...
* `Field` is an element of a finite field. Associated functions:
  ...
  * `ENCODED_SIZE` is the size of each field element in bytes.
