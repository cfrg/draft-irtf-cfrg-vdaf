...
The overall VDAF execution can be thought of in the following steps:
...
To verify the correctness of the input shares, the Aggregators interact with
one another over `ROUND` rounds.  In the first round, each aggregator produces a
single output based on its input share.  In subsequent rounds, each aggregator
is given the outputs from all aggregators in the previous round.  After the
final round, either all aggregators hold an output share (if the measurement was
valid) or all aggregators report an error (if it was not).
...
~~~~
    ...
    input_share_0  input_share_1       input_share_[SHARES-1]
      ...
      V              V                   V
    +-----------+  +-----------+       +-----------+
    | ver_init  |  | ver_init  |       | ver_init  |
    +-----------+  +------------+      +-----------+
      |              |              ...  |
      V              V                   V
    +-----------+  +-----------+       +-----------+
    | ver_next  |  | ver_next  |       | ver_next  |
    +-----------+  +-----------+       +-----------+
      |              |              ...  |
      ====================================
      |              |                   |
...
~~~~
{: #eval-flow title="VDAF verification process on the input shares for a single
...
* `verify_init(verify_param, agg_param, nonce, input_share) ->
  ValidationState` is the deterministic verification-state initialization
  algorithm run by each Aggregator to begin evaluation. Its inputs are the
  ...
  (`input_share`). Its outputs is the Aggregator's initial validation state.

* `verify_next(state, inbound: Vec[Bytes]) -> (next_state, outbound)` is the
...
The verification-state update accomplishes two tasks that are essential to most
...
states.  The aggregator enters the first state on `validation_init`, and
`validation_next` advances the aggregator to the next state.  Thus, in addition
to defining the number of rounds (`ROUNDS`), a VDAF instance must define the
state of the aggregator after each round.

Note that it is possible for a VDAF to specify `ROUNDS == 0`, in which case
each Aggregator runs the verification-state update algorithm once and immediately
...
Once an aggregator has validated output shares for a batch of measurements
...
* `output_to_aggregate_shares(output_shares)` is the deterinistic aggregation
  algorithm.  It is run by each aggregator over the output shares it has
  computed over a batch of measurement inputs.
...
* `agg_output(agg_shares) -> aggregate_result` is run by the Collector in
