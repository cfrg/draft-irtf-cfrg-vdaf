...
* `vdaf_setup() -> (public_param, verify_params: Vec[Bytes])` is the randomized
  ...
  (`verify_params`). The length of the latter MUST be `SHARES`. In general, an
  Aggregator's verification parameter is considered secret and MUST NOT be
  revealed to the Clients, Collector or other Aggregators. The parameters are
  generated once and MAY be reused across multiple VDAF evaluations. To avoid
  leaking `verify_params` to Clients, it is RECOMMENDED that the Aggregators
  generate them only after a set of Client inputs has been collected for
  verification.

## Sharding

In order to protect the privacy of its measurements, a VDAF client divides its
...
* `measurement_to_input_shares(public_param, input) -> input_shares: Vec[Bytes]`
  ...
  shares, one for each Aggregator. The length of `input_shares` MUST be
  `SHARES`.
...
with one another over `ROUND` rounds. Prior to each round, each Aggregator
constructs an outbound message. Next, the sequence of outbound messages produced
by the Aggregators is combined into a single message, called a "preparation
message". (Each of the outbound messages are called "preparation-message
shares".) Finally, the preparation message is distributed to the Aggregators to
...
* `PrepState(verify_param, agg_param, nonce, input_share)` is the deterministic
...
* `PrepState.next(inbound: Optional[Bytes]) -> outbound` is the deterministic
  preparation-state update algorithm run by each Aggregator. It updates the
  Aggregator's preparation state (an instance of `PrepState`) and returns either
  its preparation-message share for the current round or, if this is the last
  round, its output share. An exception is raised if a valid output share could
  not be recovered. The input of this algorithm is the inbound preparation
  message or, if this is the first round, `None`.

* `prep_shares_to_prep(agg_param, prep_shares: Vec[Bytes]) -> prep`
  is the deterministc preparation-message preprocessing algorithm. It combines
  the preperation-message shares generated by the Aggregators in the previous
  round into the pepration message consumed by each in the next round.
...
aggregating an invalid output share. Moreover, in protocols like Prio+ [AGJOP21]
based on oblivious transfer, it is necessary for the Aggregators to interact in
order to recover output shares at all.
...
* `aggregate_shares_to_result(agg_param, agg_shares: Vec[Bytes]) -> agg_result`
  is run by the Collector in order to compute the aggregate result from the
  Aggregators' shares. The length of `agg_shares` MUST be `SHARES`. This
...
def run_vdaf(agg_param, nonces: Vec[Bytes], input_batch: Vec[Bytes]):
  ...
  for (nonce, input) in zip(nonces, input_batch):
    ...
    for i in range(ROUNDS+1):
      ...
      inbound = prep_shares_to_prep(agg_param, outbound)
