...
conceret VDAF must provide.

## Constants and Associated Types

Each concrete VDAF specifies the following constants:

* `Vdaf.SHARES: Unsigned` is the number of Aggregators for which the VDAF is
  defined.

* `Vdaf.ROUNDS: Unsigned` is the number of rounds of communication during the
  preparation phase. (The communication pattern is derfined in
  {{sec-vdaf-prepare}}.)

In addition, the VDAF provides the following types:

* `Vdaf.Measurement` is the type measurement type used by a client during the
  sharding phase ({{sec-vdaf-shard}}).

* `Vdaf.PublicParam` is the type of public parameter used by a Client during the
  sharding phase.

* `Vdaf.VerifyParam` is the type of verification parameter used by each
  Aggregator during the preparation phase ({{sec-vdaf-prepare}}).

* `Vdaf.Prep` represents the state of an Aggregator during the preparation
  phase. This class implements the interface described in {{sec-vdaf-prepare}}.

* `Vdaf.AggParam` is the type of aggregation parameter used by each Aggregator
  during the preparation phase.

* `Vdaf.OutShare` is the type of each output share recovered by the Aggregators
  during the preparation phase.

* `Vdaf.AggShare` is the type of result of combining a sequence of output shares
  during the aggregation phase ({{sec-vdaf-aggregate}}).

* `Vdaf.AggResult` is the type of aggregate result ({{sec-vdaf-unshard}}).
...
* `Vdaf.setup() -> (Vdaf.PublicParam, Vec[Vdaf.VerifyParam])` is the randomized
  setup algorithm used to generate the public parameter used by the Clients and
  the verification parameters used by the Aggregators. The length of the latter
  MUST be equal to `Vdaf.SHARES`. In general, an Aggregator's verification
  parameter is considered secret and MUST NOT be revealed to the Clients,
  Collector or other Aggregators. The parameters MAY be reused across multiple
  VDAF evaluations. See {{security}} for a discussion of the security
  implications this has depending on the threat model.
...
* `Vdaf.measurement_to_input_shares(public_param: Vdaf.PublicParam, input:
  Vdaf.Measurement) -> Vec[Bytes]` is the randomized input-distribution
  algorithm run by each Client. It consumes the public parameter and input
  measurement and produces a sequence of input shares, one for each Aggregator.
  The length of the output MUST be `Vdaf.SHARES`.
...
over `Vdaf.ROUNDS` rounds. Prior to each round, each Aggregator constructs an
...
* `Vdaf.prep_init(verify_param: Vdaf.VerifyParam, agg_param: Vdaf.AggParam,
  nonce: Bytes, input_share: Bytes) -> Vdaf.Prep` is the deterministic
  ...
  aggregator's verification parameter (`verify_param`), the aggregation
...
* `prep.next(inbound: Optional[Bytes]) -> Union[Bytes, Vdaf.OutShare]` is the
  deterministic preparation-state update algorithm run by each Aggregator. It
  updates the Aggregator's preparation state (`prep`, an instance of
  `Vdaf.Prep`) and returns either its preparation-message share for the current
  round or, if this is the last round, its output share. An exception is raised
  if a valid output share could not be recovered. The input of this algorithm is
  the inbound preparation message or, if this is the first round, `None`.

* `Vdaf.prep_shares_to_prep(agg_param: Vdaf.VerifyParam, prep_shares:
  Vec[Bytes]) -> Bytes` is the deterministc preparation-message preprocessing
  algorithm. It combines the preperation-message shares generated by the
  Aggregators in the previous round into the pepration message consumed by each
  in the next round.

In effect, each Aggregator moves through a linear state machine with
`Vdaf.ROUNDS+1` states.  The Aggregator enters the first state on using the
initialization algorithm, and the update algorithm advances the Aggregator to
the next state. Thus, in addition to defining the number of rounds
(`Vdaf.ROUNDS`), a VDAF instance defines the state of the Aggregator after each
round.
...
* `Vdaf.output_to_aggregate_shares(agg_param: Vdaf.AggParam, output_shares:
  Vec[Vdaf.OutShare]) -> agg_share: vdaf.AggShare` is the deterministic
...
* `Vdaf.aggregate_shares_to_result(agg_param: Vdaf.AggParam, agg_shares:
  Vec[Vdaf.AggShare]) -> Vdaf.AggResult` is run by the Collector in order to
...
def run_vdaf(Vdaf,
    ...
    # Each Aggregator combines its output shares into its aggregate
    # share.
    agg_shares = []
    for j in range(Vdaf.SHARES):
...
* `Field.ENCODED_SIZE: Unsigned` is the number of bytes used to encode a field element
  as a byte string.
...
takes advantage of optimizations described later in [BBCGGI19] that improve
...
of generality as the original system.
...
Collector, who recovers the aggregate result by adding up the vectors.

The main problem that needs to be solved is to verify that the additive shares
generated by the Client add up to a valid input, i.e., that `C(x)=0`. The
solution introduced by [BBCGGI19] is what they call a zero-knowledge proof
...
results yields allows each Aggregator to decide if the input shares are valid.
...
(More on this in {{prio3-construction}}.) An FLP specifies the following core
algorithms (encoding of inputs is described in {{flp-encode}}:
...
`run_flp(input)` always returns `True`. On the other hand, if `input` is
...
algorithm run by the aggregators during preparation.
...
{{prelim}} and the helper function `expand` described in the same section.
...
The preparation state is defined as follows. It involves two additional helper
functions, `encode_prep_message` and `decode_prep_message`, both of which are defined in
{{prio3-helper-functions}}.
...
class PrepState:
  ...
  def next(self, inbound: Optional[Bytes]):
    if self.step == "ready" and inbound == None:
      ...
      self.step = "waiting"
      return encode_prep_message(
        self.k_joint_rand_share,
        self.verifier_share,
      )

    elif self.step == "waiting" and inbound != None:
      (k_joint_rand, verifier) = decode_prep_message(inbound)
...
def prep_shares_to_prep(_, inbound: Vec[Bytes]):
  ...
  for r_share in inbound:
    (k_joint_rand_share,
     verifier_share) = decode_prep_message(r_share)

    k_joint_rand ^= k_joint_rand_share
    verifer += verifier_share

  return encode_prep_message(k_joint_rand, verifier)
~~~
...
def output_to_aggregate_shares(_, output_shares: Vec[Bytes]):
  agg_share = Field.zeros(OUTPUT_LEN)
...
* `encode_helper_share(input_share, proof_share, blind, hint) -> encoded: Bytes`
  encodes a helper share as a byte string.
* `decode_leader_share` and `decode_helper_share` decode a leader and helper
  share respectively.
* `encode_prep_message(key, verifier: Vec[Field]) -> encoded` encodes a
  key (share) and verifier (share) as a byte string.
* `decode_prep_message` decodes a key (share) and verifier (share).
